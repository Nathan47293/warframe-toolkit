...
- **Arcanes** panel (profile page) — two tabs:
  ...
  - **Watchlist**: vosfor/plat hunter for a curated shortlist of arcanes. Pulls live ingame sells, scores by `vosfor/plat` (max-rank vosfor multipliers built in: ×21 for R5, ×10 for R3), filters by min vosfor/plat AND min vosfor/trade (each in-game trade fits up to 6 max-rank arcanes), and emits **every qualifying listing** as its own row (so you see all the cheap copies, not just one per arcane), each with whisper, [block], and [+rep] links.
- **Ducanator 2.0** (`/tools/ducats`) — scrapes the top-N rows of the live ducat-deal table (despite the page using react-virtuoso virtualization), pulls every qualifying ingame seller per item, and ranks them globally by your chosen metric (D/p, D/trade, or total D) so the best real deals always surface first regardless of which item they belong to.
...
- **Per-listing emission**: every qualifying seller across every watched arcane becomes its own row. The list is sorted by **total vosfor descending** (biggest haul first), with a tiebreaker on **vosfor/plat descending** (at equal payout, the better rate wins). The top-M slice happens after the global sort, so the chunkiest deals surface first regardless of which arcane they belong to. Name line still leads with `v/p` (the universal rate metric) and shows total vosfor as the secondary metric, so you always see both numbers.
...
- **Notify on new deals** (toggle, default ON): fires a Windows toast via `chrome.notifications` only when there's a **new listing** (slug:rank:seller not yet notified about) whose **vosfor/trade** strictly beats the highest still-visible already-notified listing. Tracker is pruned every scan: if a deal you were notified about disappears (got bought, delisted), the ceiling falls and the next-best deal re-qualifies for a notification — even if its v/trade is lower than what you'd previously been alerted to. Click action focuses the profile tab (where the watchlist panel lives) and copies the top-v/trade deal's whisper to clipboard.
...
- **Per-listing emission**: every qualifying (item, seller) combination becomes its own row. Sorted globally by your chosen metric (with the optional Then-by tiebreaker, then total D as the final tier so a high-quantity listing outranks a low-quantity one at the same rate). Top-M slice happens after the global sort, so the highest-rate deals always surface first regardless of which item they belong to. (Old behaviour was one-row-per-item with a "best seller" reduction; if you only care about ducats you'd rather see all the cheap copies than be hidden from them.)
...
- **Notify on new deals** (toggle, default ON): fires a Windows toast via `chrome.notifications` only when there's a **new listing** (slug:seller not yet notified about) whose **D/trade** strictly beats the highest still-visible already-notified listing. Tracker is pruned every scan: if a deal you were notified about disappears (got bought, delisted), the ceiling falls and the next-best deal re-qualifies for a notification — even if its D/trade is lower than what you'd previously been alerted to. Scans only run while a Ducanator tab is open (see the tab-presence gate above), but the tab can be hidden, throttled, or in another window — the SW alarm keeps firing on time regardless of focus state.
  - Title: `Ducanator: 3 deals · top 540D/trade` (D/trade leads the title because it's what actually scales with your trade-slot budget)
  - Body: `Mirage Prime Helmet · 90.50 D/p · 540D total`
  ...
  - Gating: a `{slug:seller → D/trade}` map persisted in `chrome.storage.local` (`wfbh-active-notified`) tracks every listing we've ever sent a notification about that's still visible in the most recent scan. Each scan prunes entries whose dealId isn't in current results (the deal got bought, delisted, or aged out), then notifies only if the best new (untracked) listing strictly beats the highest value still in the tracker. Disappearing deals naturally clear out their slot of the ceiling, so a hot 400 D/trade getting bought lets the next 350 D/trade fire — but a 350 sitting alongside an active 400 stays quiet. No timestamp-based prune; no manual reset needed.
