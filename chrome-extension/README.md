...
- **Per-listing emission**: every qualifying seller across every watched arcane becomes its own row. The list is sorted by vosfor/plat descending, with a tiebreaker on total vosfor (so a `7p × 13` listing outranks a `7p × 3` one at the same rate). The top-M slice happens after the global sort, so the highest-rate deals always surface first regardless of which arcane they belong to.
