...
- **Notify on new deals** (toggle, default ON): fires a Windows toast via `chrome.notifications` only when this scan's max **vosfor/trade** strictly beats the previous scan's. The bar updates every scan (notify or not), so after a peak deal disappears the threshold drops to whatever the current top is and the next improvement re-notifies. Click action focuses the profile tab (where the watchlist panel lives) and copies the top-v/trade deal's whisper to clipboard.
...
- **Notify on new deals** (toggle, default ON): fires a Windows toast via `chrome.notifications` only when this scan's max **D/trade** strictly beats the previous scan's. The bar advances every scan (notify or not), so after a peak deal disappears the threshold drops to the current top and the next improvement re-notifies. Scans only run while a Ducanator tab is open (see the tab-presence gate above), but the tab can be hidden, throttled, or in another window — the SW alarm keeps firing on time regardless of focus state.
  ...
  - Gating: persisted as a single number in `chrome.storage.local` (`wfbh-prev-top-dpt`); current scan's max D/trade must strictly exceed it to notify. The number is overwritten with this scan's max regardless of notify outcome, so the bar always reflects the most recent observation. No slug:seller bookkeeping; no per-deal dedup map.
