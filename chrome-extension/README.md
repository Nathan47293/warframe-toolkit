...
- **Notify on new deals** (toggle, default ON): fires a Windows toast via `chrome.notifications` when at least one listing in current results isn't in the slug:rank:seller dedup map (1h TTL). Body shows the highest-v/trade listing in the scan as the subject. Click action focuses the profile tab and copies that listing's whisper to clipboard. Tune your input filters tightly (Min v/p, Min v/trade) so every fresh notification is meaningful.
...
- **Notify on new deals** (toggle, default ON): fires a Windows toast via `chrome.notifications` when at least one listing in current results isn't in the slug:seller dedup map (1h TTL). Tune your input filters (Min D/p, Min D/trade, Min total D, tiered floors) tightly so every fresh notification is meaningful. Scans only run while a Ducanator tab is open (see the tab-presence gate above), but the tab can be hidden, throttled, or in another window — the SW alarm keeps firing on time regardless of focus state.
  ...
  - Gating: a `{slug:seller → lastSeenTs}` dedup map in `chrome.storage.local` (`wfbh-notified-deals`). At each scan: prune entries older than 1h, then notify if any current listing's dealId isn't in the (pruned) map. After notifying, refresh the timestamp on every listing currently in scan results (so long-lived listings keep their slot through the next prune instead of falling out and re-triggering). Same shape on the watchlist side (`wfaa-wl-notified-deals` in localStorage).
